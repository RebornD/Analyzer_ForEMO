...
5. HyperVolume やIGDの計算プログラムはtoolbox/Calclatorディレクトリにあるが，意図的にこのプロジェクトには省いている．同じ研究室の学生ならいつものあれで通じると思う．IGD計算プログラムは自分で書いたが，コマンドライン解析に外部のsrcを使用したのでgitには上げない．
...
start.py
	メインスクリプト，コマンドラインを使用して実行内容を決定する．
